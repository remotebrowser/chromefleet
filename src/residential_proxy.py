...
class MassiveLocation(BaseModel):
    ...
    def validate_and_normalize(self) -> Self:
        ...
        self.subdivision = (str(self.subdivision) if self.subdivision else "").lower().strip().replace(" ", "_")
        ...
        if self.country != "us":
            ...
            if self.subdivision:
                raise ValueError(f"subdivision not supported for non-US (country: '{self.country}')")
